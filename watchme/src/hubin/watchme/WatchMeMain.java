...
import hubin.watchme.config.Configure;
import hubin.watchme.config.WatchMeAppAttributes;
import hubin.watchme.event.WindowTracker;
import hubin.watchme.gui.About;
import hubin.watchme.gui.Clear;
import hubin.watchme.gui.ClearAll;
import hubin.watchme.gui.Close;
import hubin.watchme.gui.ComponentBuilder;
import hubin.watchme.gui.Debug;
import hubin.watchme.gui.Delete;
import hubin.watchme.gui.DeleteAll;
import hubin.watchme.gui.DndFileOpener;
import hubin.watchme.gui.Edit;
import hubin.watchme.gui.Exit;
import hubin.watchme.gui.FileFollowingPane;
import hubin.watchme.gui.FollowAppAction;
import hubin.watchme.gui.Menu;
import hubin.watchme.gui.Open;
import hubin.watchme.gui.Pause;
import hubin.watchme.gui.PopupMenu;
import hubin.watchme.gui.Reset;
import hubin.watchme.gui.StartupStatus;
import hubin.watchme.gui.TabbedPane;
import hubin.watchme.gui.ToolBar;
import hubin.watchme.gui.FollowAppAction.ActionContext;
import hubin.watchme.nav.Bottom;
import hubin.watchme.nav.NextTab;
import hubin.watchme.nav.PreviousTab;
import hubin.watchme.nav.Top;
import hubin.watchme.search.ClearAllHighlights;
import hubin.watchme.search.ClearHighlights;
import hubin.watchme.search.Find;
import hubin.watchme.search.SearchableTextPane;
import hubin.watchme.systeminterface.DefaultSystemInterface;
...
public class WatchMeMain
{
	...
	private static Logger LOG = Logger.getLogger(WatchMeMain.class.getName());
	...
	private WatchMeAppAttributes attributes;
	private static WatchMeMain instance;
	private static ResourceBundle resources = ResourceBundle
	        .getBundle("hubin.watchme.FollowAppResourceBundle");
	 ...
	 */
	WatchMeMain(List<String> fileNames) throws IOException, InterruptedException,
	        InvocationTargetException
	{
		this(fileNames, null);
	}

	WatchMeMain(List<String> filenames, File propertyFile) throws IOException, InterruptedException,
	        InvocationTargetException
	{
		...
		// load the attributes
		attributes = new WatchMeAppAttributes(propertyFile);

		// add listeners to update the recent files list
		RecentFileListener rfl = new RecentFileListener();
		attributes.addPropertyChangeListener(WatchMeAppAttributes.RECENT_FILES_KEY, rfl);
		attributes.addPropertyChangeListener(WatchMeAppAttributes.RECENT_FILES_MAX_KEY, rfl);
	...
	public WatchMeAppAttributes getAttributes()
	{
		return attributes;
	}
	...
	public static WatchMeMain getInstance()
	{
		return instance;
	}
	...
	public static void main(String[] args)
	{
		try
		{
			...
			instance = new WatchMeMain(fileNames, propFile);
			SwingUtilities.invokeAndWait(new Runnable()
			{
				public void run()
				{
		...
		public void propertyChange(PropertyChangeEvent evt)
		{
			if (recentFilesMenu != null)
			{
				...
				for (int i = recentFiles.size() - 1; i >= 0; i--)
				{
					...
					// context of
					// the inner class
					recentFilesMenu.add(new Open(WatchMeMain.this, recentFiles.get(i)));
				}
			}
		}
	}
